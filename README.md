# ⚡️ RAG-FZX: Enterprise-Grade Async RAG System
...
**RAG-FZX** 是一个**高性能、模块化、可扩展**的本地知识库问答解决方案。本项目按**企业级 SaaS 标准**构建，旨在解决 RAG 系统在生产环境中的**高并发阻塞**、**复杂表格解析**与**回答精准度**问题。
...
```bash
conda create -n rag_fzx python=3.11
...
# 输出: data/FZX_Corp_Financial_Report_2024.pdf
```
...
```bash
python -m src.rag_fzx.cli ingest --force-rebuild
...
```bash
uvicorn src.rag_fzx.api.main:app --host 0.0.0.0 --port 8000 --reload
...
```text
rag_fzx/
├── 🌐 src/rag_fzx/api/       # FastAPI 接口层 (Main, Schemas)
├── 💼 src/rag_fzx/services/  # 业务逻辑 (Query, Ingestion)
├── 🧩 src/rag_fzx/components/# 核心组件 (PDFLoader, LLM, FAISS)
│   └── pdf_loader.py         # [核心] Docling 适配器
├── ⚛️ src/rag_fzx/core/      # 接口定义 (Async Interfaces)
├── 📜 scripts/               # 评估与工具脚本
